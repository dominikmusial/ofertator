"""Decathlon (Mirakl) marketplace provider implementation"""
...
from .config import DECATHLON_CONFIG
import logging
...
class DecathlonMarketplaceClient:
    """
    Decathlon marketplace client using shared Mirakl infrastructure.
    ...
    def __init__(self, api_key: str, shop_id: Optional[str] = None):
        ...
        # Compose Mirakl API client with Decathlon config
        self._api_client = MiraklAPIClient(
            api_key=self.api_key,
            shop_id=self.shop_id,
            base_url=self._config.base_url
        )
        logger.info(f"Initialized DecathlonMarketplaceClient with shop_id: {self.shop_id}")
    
    # ===== Core Methods =====
    
    def create_offer(self, offer_data: dict) -> dict:
        """
        Create new offer on Decathlon/Mirakl
    ...
    def get_offer(self, offer_id: str) -> dict:
        """
        Get offer details from Decathlon/Mirakl
    ...
    def update_offer(self, offer_id: str, updates: dict) -> dict:
        """
        Update existing offer on Decathlon/Mirakl
    ...
    def upload_image(self, image_bytes: bytes, filename: str) -> str:
        """
        Upload image to Decathlon/Mirakl
        
        Note: Mirakl typically doesn't have a direct image upload API
        Images are usually hosted externally and referenced by URL
        """
        raise NotImplementedError(
            "Mirakl image upload not implemented - host images externally and use URLs"
        )
    
    def get_categories(self, parent_id: Optional[str] = None) -> List[dict]:
        """
        Get categories from Decathlon/Mirakl
    ...
    def get_marketplace_type(self) -> MarketplaceType:
        ...
        return MarketplaceType.decathlon
    
    def get_capabilities(self) -> dict:
        """Return Decathlon/Mirakl capabilities"""
        return {
    ...
    # Decathlon-specific methods (not in interface)
    ...
    def get_orders(
        ...
        """
        Decathlon-specific: Get orders for reporting
        ...
        Args:
            ...
            channel_code: Channel code filter (note: not directly supported by OR11)
