"""Decathlon (Mirakl) API endpoints"""
...
from app.infrastructure.marketplaces.decathlon.config import DECATHLON_CONFIG
...
async def authorize_decathlon_account(
    api_key: str = Form(...),
    ...
    """
    Add Decathlon account using API key authentication
    ...
    """
    try:
        logger.info(f"Authorizing Decathlon account for user {current_user.id}")
        ...
        result = mirakl_auth.validate_credentials(api_key, shop_id, DECATHLON_CONFIG.base_url)
        
        logger.info(f"Decathlon credentials validated: {result['account_name']}")
        ...
        logger.info(f"Created Decathlon account: {account.id}")
        ...
        if current_user.role in [UserRole.vsprint_employee, UserRole.admin]:
            ...
            logger.info(f"Auto-sharing Decathlon account {account.id} with vsprint team")
        
        db.add(user_account)
        
        db.commit()
        
        logger.info(f"Successfully added Decathlon account {account.id} for user {current_user.id}")
        ...
        db.rollback()
        logger.error(f"Failed to authorize Decathlon account: {e}")
