"""Castorama (Mirakl) API endpoints"""
...
from app.infrastructure.marketplaces.castorama.config import CASTORAMA_CONFIG
...
async def authorize_castorama_account(
    api_key: str = Form(...),
    ...
    """
    Add Castorama account using API key authentication
    ...
    """
    try:
        logger.info(f"Authorizing Castorama account for user {current_user.id}")
        ...
        result = mirakl_auth.validate_credentials(api_key, shop_id, CASTORAMA_CONFIG.base_url)
        
        logger.info(f"Castorama credentials validated: {result['account_name']}")
        ...
        logger.info(f"Created Castorama account: {account.id}")
        ...
        if current_user.role in [UserRole.vsprint_employee, UserRole.admin]:
            ...
            logger.info(f"Auto-sharing Castorama account {account.id} with vsprint team")
        
        db.add(user_account)
        
        db.commit()
        
        logger.info(f"Successfully added Castorama account {account.id} for user {current_user.id}")
        ...
        db.rollback()
        logger.error(f"Failed to authorize Castorama account: {e}")
